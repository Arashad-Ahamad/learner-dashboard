...
describe('LearnerProgressBadge', () => {
  it('exposes an accessible name including title, status, and progress', () => {
    render(
      <LearnerProgressBadge status="in-progress" title="JavaScript Fundamentals" progress={65} />
    );
    expect(
      screen.getByRole('button', { name: 'JavaScript Fundamentals, In Progress, 65% complete' })
    ).toBeInTheDocument();
  });
  ...
  it('calls onClick when activated with the mouse', async () => {
    ...
    render(<LearnerProgressBadge title="Course" progress={0} onClick={onClick} />);
  ...
  it('is not focusable or clickable when disabled', async () => {
    const onClick = vi.fn();
    const user = userEvent.setup();
    render(<LearnerProgressBadge status="disabled" title="Locked Course" progress={30} onClick={onClick} />);
  ...
  it('clamps the visual fill width for out-of-range progress values', () => {
    const { container } = render(<LearnerProgressBadge title="Course" progress={150} />);
    // The inner fill bar's inline width style must be clamped to 100%,
    // even though 150 is echoed verbatim to aria-valuenow above.
    const fillBar = Array.from(container.querySelectorAll<HTMLElement>('[style]')).find(el =>
      el.style.width && el.style.backgroundColor
    );
